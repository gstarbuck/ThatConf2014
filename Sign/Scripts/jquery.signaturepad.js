...
function SignaturePad (selector, options) {
  ...
  function validateForm () {
    ...
    if (valid) {
        //note:toDataURL must be implemented in the browser
        var x = self.getSignatureImage();

        $('#sigImage').attr('src', x);
        $('#sigJson').text(JSON.stringify(output));
        $('#imgData').text(x);
    }
    ...
    if (element.getContext) {
      ...
      if (!settings.displayOnly) {
        ...
        if (settings.validateFields) {
          if ($(selector).is('form')) {
              $(selector).bind('submit.signaturepad', function (e) { e.preventDefault(); return validateForm(); })
          } else {
              $(selector).parents('form').bind('submit.signaturepad', function () { e.preventDefault(); return validateForm(); })
          }
        }
